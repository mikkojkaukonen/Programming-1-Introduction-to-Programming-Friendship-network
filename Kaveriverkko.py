...
Firstname Lastname: Mikko Kaukonen
...
def read_friendship_network(filename):
    ...
    #       Dictionary which contains all the relevant information concerning
    #       the friendships when used later in the program.
...
def add_friendship(network, name1, name2):
    ...
    for name in [ name1, name2 ]:
        if not name.isalpha():
            ...
            return
#
#The rest of this function is self-made
#

    # Sanity check here to make sure no one can be added as his or her own
    # friend

    if name1 == name2:
        return

    # Adds information about <name1> being <name2>'s friend and
...
#
    ...
    # Prints the information of all friendships in the alphabetichal order
...
#
    ...
    # Checks if <name> is an unknown person. If so print an error
    # message and stop.
...
#
    ...
    # Checks that a person does not have common friends with him/herself
    # and if so print a message and stop.

    if name1 == name2:
        print(f"{name1} has no common friends with him/herself.")

    # Finds the common friends of <name1> and <name2>
    # and print them on the screen in an alphabetical order.
    # If <name1> and <name2> do not have common friends,
    # prints instead an error message.

    else:

        common_friends = []

        for name in network[name1]:
            if name in network[name2]:
